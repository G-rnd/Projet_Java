...
public class Membre {
    ...
    private final ArrayList<Operation> listeDefraiements = new ArrayList<>();
     ...
     */
    public int getNbVisitesRestantes() {
    ...
    protected void removeNbVisites() {
        if (this.getNbVisitesRestantes() > 0) {
            this.nbVisites--;
        }
    }
    ...
    protected void setNbVisites(int n) {
        if (n < nbVisitesMax && n >= 0) {
            this.nbVisites = n;
        } else {
            System.out.println("[Membre] : Le nombre de visites saisi est incorrecte.");
        }
    }
     ...
     */
    public void setCotisation(boolean b) {
        this.cotisation = b;
    }
    ...
    protected void voterArbre(ArbreAssociation a) {
        try {
            if (a != null) {
                ...
                throw new Exception("Arbre introuvable;");
            }
        } catch (Exception e) {
            System.out.println("[Membre] : Le vote a échoué : " + e.toString());
        }
    }
    ...
    protected void remplacerVoteArbre(Association a, int idAncien, int idNouveau) {
        try {
            if (a.getArbreById(idNouveau).getArbre().getEstRemarquable() || idAncien == idNouveau ||
     ...
     */
    protected int getNbVotesRestants() {
        return this.nbVotes;
    }
     ...
     *
     * @return Le nombre de votes effectués par un membre qui n'entraîneront pas un effacement d'anciens votes.
     */
    protected int getNbVotes() {
        return nbVotesMax - this.nbVotes;
    }
    ...
    /**
     * Ajoute la note de défraiement à la liste des défraiements du membre.
     *
     * @param op Le défraiement
     */
    public void ajouterDefraiement(Operation op) {
        if (op.getIsRecette() && op.getMontant() > 0)
            listeDefraiements.add(op);
        else
            System.out.println("[Membre] : Le défraiement a échoué pour le membre.");
    }

    /**
     * Permet de masquer le nom de la personne dans une note de défraiement.
     */
    public void masquerDefraiements() {
        for (Operation op : listeDefraiements)
            op.setEntite("\u001C personne ayant quitté l’association \u001D");
    }

    /**
     * Retourne une chaîne de caractères composé du prénom et du nom du membre.
     *
     * @return Le prénom et le nom du membre.
     */
    public String getPrenomNom() {
        return p.getPrenomNom();
    }
